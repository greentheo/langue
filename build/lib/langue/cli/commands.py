...
def register_activity_commands(main_group):
    ...
    @click.option("--difficulty", "-d", type=int, default=1, help="Difficulty level (1-5)")
    @click.pass_context
    def flashcards(ctx, language: Optional[str], count: int, difficulty: int):
        ...
        # If no level in profile, map from difficulty
        if not level:
        ...
        activity = FlashcardActivity(
            ...
            model_name=f"claude:claude-3-haiku-20240307",
            level=level,
    ...
    @main_group.command()
    @click.option("--language", "-l", help="Language to practice")
    @click.option("--difficulty", "-d", type=int, default=1, help="Complexity level (1-5)")
    ...
    # Modify the existing library_command to use Claude by default
