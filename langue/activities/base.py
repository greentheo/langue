...
class Activity(abc.ABC):
    ...
    def __init__(self, language: str, difficulty: int = 1, model_name: Optional[str] = None, level: Optional[str] = None):
        ...
        # Handle level parameter, defaulting to difficulty mapping if not provided
        if level:
            self.level = level.lower()
        else:
            # Map difficulty to level if not provided
            level_map = {1: "a1", 2: "a2", 3: "b1", 4: "b2", 5: "c1", 6: "c2"}
            self.level = level_map.get(difficulty, "a1")

        self.points_earned = 0
        self.words_encountered = set()

        # Initialize points and word tracking
