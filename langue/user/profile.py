...
class UserProfile:
    ...
    def __init__(
        ...
        self.current_level = current_level.lower() if current_level else "a1"
        ...
        if self.current_language not in self.language_levels:
            ...
            # Keep language_levels and current_level in sync
            self.current_level = self.language_levels[self.current_language]
...
class UserProfileManager:
    ...
    def load_user(self) -> UserProfile:
        ...
        """
        print(f"DEBUG: Loading user profile with ID: {self._default_user_id}")
        try:
            ...
            if user:
                print(f"DEBUG: Loaded user from database: {user.username}, level={user.current_level}, language_levels={user.language_levels}")
                return user
            else:
                print("DEBUG: User not found in database")
        except ImportError:
            # If storage module is not available yet, skip database loading
            print("DEBUG: Storage module not available, skipping database load")
            pass

        # If not in database, try legacy file storage
        if self.user_file.exists():
            print(f"DEBUG: Trying to load from file: {self.user_file}")
            try:
                with open(self.user_file, "r", encoding="utf-8") as f:
                    user_data = json.load(f)
                user = UserProfile.from_dict(user_data)
                print(f"DEBUG: Loaded user from file: {user.username}, level={user.current_level}, language_levels={user.language_levels}")
                ...
                print(f"DEBUG: Error loading user profile from file: {e}")

        # Create new user if not found
        print(f"DEBUG: Creating new user with ID: {self._default_user_id}")
        new_user = UserProfile(user_id=self._default_user_id)
        print(f"DEBUG: New user created with level={new_user.current_level}, language_levels={new_user.language_levels}")
        return new_user
    ...
    def track_activity(self, activity_type: str, language: Optional[str] = None,
        ...
        # Update the user's language levels if needed
        if lang not in user.language_levels:
            user.language_levels[lang] = user.current_level
    ...
    def _check_achievements(self, user: UserProfile, activity_type: str) -> None:
        ...
        Args:
            ...
            activity_type: Type of activity completed
        """
        # Ensure user level settings are consistent
        if user.current_language in user.language_levels:
            # Keep user.current_level and language_levels in sync
            if user.current_level != user.language_levels[user.current_language]:
                print(f"DEBUG: Fixing inconsistent level settings - setting language_levels[{user.current_language}] from {user.language_levels[user.current_language]} to {user.current_level}")
                user.language_levels[user.current_language] = user.current_level
